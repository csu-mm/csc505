...
class Mondal:
    ...
    def __init__(self):
        Mondal.registry.append(Requirement("Requirement"))
        Mondal.registry.append(Design("Design"))
        Mondal.registry.append(Development("Development"))
        Mondal.registry.append(Testing("Testing"))
...
class Requirement(WaterfallModelPhase):
    tasks = [
             "1. Understand and gather the exact requirements of the customer.", 
             "2. Analyze these requirements.",
             "3. Document these analyzed requirements in software requirement specification (SRS) document.",
             "4. SRS document serves as a contract between the development team and customers."
            ]
    def getTasks(self):
        return "\n".join(self.tasks)


class Design(WaterfallModelPhase):
    tasks = [
             "1. Convert the SRS document into a format, called Software Design Document(SDD).", 
             "2. SDD can be coded in a programming language.",
...
class Development(WaterfallModelPhase):
    tasks = [
...
class Deployment(WaterfallModelPhase):
    tasks = [
             ...
             "3. End-user training, setting up necessary environments, and ensuring everything is running smoothly."
            ]
    def getTasks(self):
        return "\n".join(self.tasks)
